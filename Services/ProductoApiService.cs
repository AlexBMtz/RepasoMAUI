...
public class ProductoApiService
{
    ...
    public async Task<(Producto producto, string error)> ObtenerProductoPorIdAsync(string id, string url = "https://fakestoreapi.com/products")
    {
        try
        {
            var dto = await _http.GetFromJsonAsync<ProductoApiDto>($"{url}/{id}");
